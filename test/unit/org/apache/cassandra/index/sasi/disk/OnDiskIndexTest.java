...
public class OnDiskIndexTest
{
    ...
    private static final ClusteringComparator emptyClusteringComparator = new ClusteringComparator();
    @BeforeClass
    ...
    public void testStringSAConstruction() throws Exception
    {
        ...
        OnDiskIndexBuilder builder = new OnDiskIndexBuilder(UTF8Type.instance, emptyClusteringComparator, UTF8Type.instance, OnDiskIndexBuilder.Mode.CONTAINS);
        ...
        OnDiskIndex onDisk = new OnDiskIndex(index, emptyClusteringComparator, UTF8Type.instance, new KeyConverter());
    ...
    public void testIntegerSAConstruction() throws Exception
    {
        ...
        OnDiskIndexBuilder builder = new OnDiskIndexBuilder(UTF8Type.instance, emptyClusteringComparator, Int32Type.instance, OnDiskIndexBuilder.Mode.PREFIX);
        for (Map.Entry<ByteBuffer, TokenTreeBuilder> e : data.entrySet())
            addAll(builder, e.getKey(), e.getValue());
        ...
        OnDiskIndex onDisk = new OnDiskIndex(index, emptyClusteringComparator, Int32Type.instance, new KeyConverter());
        ...
        OnDiskIndexBuilder iterTest = new OnDiskIndexBuilder(UTF8Type.instance, emptyClusteringComparator, Int32Type.instance, OnDiskIndexBuilder.Mode.PREFIX);
        for (int i = 0; i < iterCheckNums.size(); i++)
            iterTest.add(iterCheckNums.get(i), keyAt((long) i), i, Clustering.EMPTY);
        ...
        onDisk = new OnDiskIndex(iterIndex, emptyClusteringComparator, Int32Type.instance, new KeyConverter());
    ...
    {
        OnDiskIndexBuilder builder = new OnDiskIndexBuilder(UTF8Type.instance, emptyClusteringComparator, LongType.instance, OnDiskIndexBuilder.Mode.SPARSE);

        final long start = System.currentTimeMillis();
        final int numIterations = 100000;

        for (long i = 0; i < numIterations; i++)
            builder.add(LongType.instance.decompose(start + i), keyAt(i), i, Clustering.EMPTY);
        ...
        OnDiskIndex onDisk = new OnDiskIndex(index, emptyClusteringComparator, LongType.instance, new KeyConverter());
    ...
    public void testNotEqualsQueryForStrings() throws Exception
    {
        ...
        OnDiskIndexBuilder builder = new OnDiskIndexBuilder(UTF8Type.instance, emptyClusteringComparator, UTF8Type.instance, OnDiskIndexBuilder.Mode.PREFIX);
        for (Map.Entry<ByteBuffer, TokenTreeBuilder> e : data.entrySet())
            addAll(builder, e.getKey(), e.getValue());
    ...
    public void testRangeQueryWithExclusions() throws Exception
    {
        ...
        for (long i = lower; i <= upper; i++)
            builder.add(LongType.instance.decompose(i), keyAt(i), i, Clustering.EMPTY);
    ...
    public void testDescriptor() throws Exception
    {
        ...
        OnDiskIndexBuilder builder1 = new OnDiskIndexBuilder(UTF8Type.instance, emptyClusteringComparator, Int32Type.instance, OnDiskIndexBuilder.Mode.PREFIX);
        OnDiskIndexBuilder builder2 = new OnDiskIndexBuilder(UTF8Type.instance, emptyClusteringComparator, Int32Type.instance, OnDiskIndexBuilder.Mode.PREFIX);
        for (Map.Entry<ByteBuffer, Pair<DecoratedKey, Long>> e : data.entrySet())
        {
            ...
            builder1.add(e.getKey(), key, position, Clustering.EMPTY);
            builder2.add(e.getKey(), key, position, Clustering.EMPTY);
        }
        ...
        OnDiskIndex onDisk1 = new OnDiskIndex(index1, emptyClusteringComparator, Int32Type.instance, new KeyConverter());
        OnDiskIndex onDisk2 = new OnDiskIndex(index2, emptyClusteringComparator, Int32Type.instance, new KeyConverter());
    ...
    public void testSuperBlocks() throws Exception
    {
        ...
        OnDiskIndexBuilder builder = new OnDiskIndexBuilder(UTF8Type.instance, emptyClusteringComparator, Int32Type.instance, OnDiskIndexBuilder.Mode.SPARSE);
    ...
    public void testSuperBlockRetrieval() throws Exception
    {
        ...
        OnDiskIndex onDiskIndex = new OnDiskIndex(index, emptyClusteringComparator, LongType.instance, new KeyConverter());
    ...
    public void putAll(SortedMap<Long, TokenTreeBuilder.Entries> offsets, TokenTreeBuilder ttb)
    {
        for (Pair<Long, TokenTreeBuilder.Entries> entry : ttb)
    ...
    {
        OnDiskIndexBuilder builderA = new OnDiskIndexBuilder(UTF8Type.instance, emptyClusteringComparator, LongType.instance, OnDiskIndexBuilder.Mode.PREFIX);
        OnDiskIndexBuilder builderB = new OnDiskIndexBuilder(UTF8Type.instance, emptyClusteringComparator, LongType.instance, OnDiskIndexBuilder.Mode.PREFIX);

        TreeMap<Long, TreeMap<Long, TokenTreeBuilder.Entries>> expected = new TreeMap<>();

        for (long i = 0; i <= 100; i++)
        {
            TreeMap<Long, TokenTreeBuilder.Entries> offsets = expected.get(i);
            if (offsets == null)
                expected.put(i, (offsets = new TreeMap<>()));

            builderA.add(LongType.instance.decompose(i), keyAt(i), i, Clustering.EMPTY);
        ...
        for (long i = 50; i < 100; i++)
        {
            ...
            builderB.add(LongType.instance.decompose(i), keyAt(position), position, Clustering.EMPTY);
        ...
        OnDiskIndex a = new OnDiskIndex(indexA, emptyClusteringComparator, LongType.instance, new KeyConverter());
        OnDiskIndex b = new OnDiskIndex(indexB, emptyClusteringComparator, LongType.instance, new KeyConverter());

        RangeIterator<OnDiskIndex.DataTerm, CombinedTerm> union = OnDiskIndexIterator.union(a, b);

        TreeMap<Long, TreeMap<Long, TokenTreeBuilder.Entries>> actual = new TreeMap<>();
        while (union.hasNext())
        {
            ...
            TreeMap<Long, TokenTreeBuilder.Entries> offsets = actual.get(composedTerm);
            if (offsets == null)
        ...
        OnDiskIndexBuilder combined = new OnDiskIndexBuilder(UTF8Type.instance, emptyClusteringComparator, LongType.instance, OnDiskIndexBuilder.Mode.PREFIX);
        combined.finish(Pair.create(keyAt(0).getKey(), keyAt(100).getKey()), indexC, new CombinedTermIterator(a, b));

        OnDiskIndex c = new OnDiskIndex(indexC, emptyClusteringComparator, LongType.instance, new KeyConverter());
    ...
    {
        TokenTreeBuilder builder = new DynamicTokenTreeBuilder(emptyClusteringComparator);

        for (final Long key : keys)
        {
            DecoratedKey dk = keyAt(key);
            builder.add((Long) dk.getToken().getTokenValue(), key, Clustering.EMPTY);
        }
    ...
    private static Set<DecoratedKey> convert(TokenTreeBuilder offsets)
    {
        ...
        Iterator<Pair<Long, TokenTreeBuilder.Entries>> offsetIter = offsets.iterator();
        while (offsetIter.hasNext())
        {
            TokenTreeBuilder.Entries v = offsetIter.next().right;

            for (TokenTreeBuilder.Entry offset : v)
    ...
    {
        for (Pair<Long, TokenTreeBuilder.Entries> token : tokens)
        {
            for (TokenTreeBuilder.Entry entry : token.right)
                builder.add(term, keyAt(entry.partitionOffset()), entry.partitionOffset(), Clustering.EMPTY);
        }
    }
